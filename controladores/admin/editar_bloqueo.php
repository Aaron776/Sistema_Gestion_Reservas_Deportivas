...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['id_cancha']) && isset($_POST['id_horario']) && isset($_POST['motivo']) && isset($_POST['fecha']) && isset($_POST['id_bloqueo'])){
     // Validar token CSRF
    ...
    $id_bloqueo = (int)Crypto::decrypt(urldecode($_POST['id_bloqueo']));
    ...
    // 1. Verificar que el bloqueo que se intenta editar realmente exista
    if(empty($errores)){
        $sql = $conexion->prepare("SELECT id FROM bloqueos WHERE id = :id_bloqueo limit 1");
        $sql->bindParam(":id_bloqueo", $id_bloqueo, PDO::PARAM_INT);
        $sql->execute();
        if(!$sql->fetch(PDO::FETCH_OBJ)){
            $errores[]="El bloqueo que intenta editar no existe en el sistema.";
        }
    }

    // 2. Verificar si tus cambios chocan con OTRO bloqueo (Excluyendo el ID actual)
    if(empty($errores)){
        $sql = $conexion->prepare("SELECT id FROM bloqueos WHERE id != :id_bloqueo AND cancha_id = :cancha_id AND horario_id = :horario_id AND fecha = :fecha limit 1");
        $sql->bindParam(":id_bloqueo", $id_bloqueo, PDO::PARAM_INT);
        $sql->bindParam(":cancha_id", $id_cancha, PDO::PARAM_INT);
        $sql->bindParam(":horario_id", $id_horario, PDO::PARAM_INT);
        $sql->bindParam(":fecha", $fecha, PDO::PARAM_STR);
        $sql->execute();
        $bloqueo_duplicado = $sql->fetch(PDO::FETCH_OBJ);
        if($bloqueo_duplicado){
            $errores[]="Ya existe otro bloqueo registrado para esa cancha en esa fecha y horario.";
        }
    }
    ...
    // Si no hay errores, procedemos a editar en la base de datos
    if(empty($errores)){
        try{
            $conexion->beginTransaction();
            // 1. Editar Bloqueo
            $sql=$conexion->prepare("UPDATE bloqueos SET cancha_id=:cancha_id,fecha=:fecha,horario_id=:horario_id,motivo=:motivo WHERE id=:id_bloqueo");
            $sql->bindParam(":cancha_id", $id_cancha, PDO::PARAM_INT);
            $sql->bindParam(":horario_id", $id_horario, PDO::PARAM_INT);
            $sql->bindParam(":fecha", $fecha, PDO::PARAM_STR);
            ...
            $mensaje_notif = "Se ha editado un bloqueo para la cancha " . $nombre_cancha . " el día " . $fecha . ". Motivo: " . $motivo;
            ...
            $conexion->commit();
            $_SESSION['exito']="Bloqueo editado exitosamente";
            header("Location: ../../admin/editar_bloqueo.php?id_bloqueo=".urlencode(Crypto::encrypt($id_bloqueo)));
            exit();
        }catch(PDOException $e){
            if($conexion->inTransaction()) $conexion->rollBack();
            error_log("Error al editar bloqueo: " . $e->getMessage());
            $errores[]="Error crítico al editar el bloqueo.";
